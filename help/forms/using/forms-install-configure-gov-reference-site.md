...
1. 搜尋&#x200B;**表單使用者**、選取&#x200B;**表單超級使用者**&#x200B;群組，並將&#x200B;**We.Gov Forms使用者**&#x200B;群組新增為成員，以重複步驟2到7。
...
1. 設定服務以連線至您選擇的SMTP伺服器：
   ...
   1. **寄件者地址**： aemformsdemo@adobe.com
...
1. 瀏覽到:
   ...
   *https://&lt;aemserver>：&lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >若要避免設定問題，請確定用來存取AEM伺服器的URL與Adobe Sign OAuth重新導向URI中設定的URL相符。
   >例如，*https://&lt;aemserver>：&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*

1. 選取&#x200B;**We.gov Adobe Sign**&#x200B;設定。
1. 按一下&#x200B;**屬性**。
1. 瀏覽至「設定」標籤。
1. 輸入oAuth URL。 例如，`https://secure.na1.echosign.com/public/oauth` （僅供說明用途）。
...
1. 在頂端導覽列中，按一下&#x200B;**銷售**&#x200B;或您自己的執行個體型別，然後&#x200B;**選取設定**。
1. 在右下角附近，按一下&#x200B;**自訂**&#x200B;和&#x200B;**開發人員資源**。
1. 尋找服務根URL。 例如，

   `https://msdynamicsserver.api.crm3.dynamics.com/api/data/v9.1/`

1. 有關重新整理和存取權杖URL的詳細資訊，請前往：
...
1. 導覽至&#x200B;**更新Source**&#x200B;標籤。
1. 確定&#x200B;**內容感知設定**&#x200B;已設為`/conf/we-gov`，且設定的資料來源為`ms-dynamics-odata-cloud-service`。
...
1. 測試服務，確保它們成功連線至已設定的資料來源。
   ...
   >[!NOTE]
   >
   >據報導，資料來源必須重新啟動AEM伺服器，才能成功繫結至FDM 。

   >[!NOTE]
   >
   >Adobe建議您使用`Ctrl + C`命令重新啟動SDK。 使用替代方法重新啟動AEM SDK。 例如，停止Java程式可能會導致AEM開發環境不一致。

#### 測試Forms資料模型(Derby) {#test-fdm-derby}

雲端設定完成後，您可能會想要測試Forms資料模型。
...
1. [測試服務](work-with-form-data-model.md#test-data-model-objects-and-services)，確保它們成功連線到設定的Data Source
   ...
   * 若要測試連線，請選取&#x200B;**HOMOMORTGAGEACCOUNT**&#x200B;並提供get服務。 測試服務，系統管理員可檢視正在擷取的資料。
...
找到Adobe Analytics帳戶的使用者認證，帳戶管理員必須執行這些工作。
...
1. 使用者名稱內有冒號。 冒號左側的所有資訊是使用者名稱，冒號右側的所有資訊是公司名稱，如以下範例所示：
...
>[!NOTE]
>
>如果您安裝`we-gov-forms.ui.analytics-<version>.zip`套件，AEM Forms Analytics資料可以離線使用，也可以不使用Adobe Analytics Cloud設定。 AEM Sites資料需要主動式雲端設定。
...
1. 在本頁中，您會看到從Adobe Analytics擷取的資訊。 此資訊與AEM Sites頁面有關。 根據設計，此資訊會定期從Adobe Analytics重新整理，且不會即時顯示。
...
1. 從相同功能表選取&#x200B;**檢視設定**，並從&#x200B;**Analytics**&#x200B;區段中選取您要顯示的資料行。
...
>[!NOTE]
>
>AEM Forms Analytics資料可在離線或沒有Adobe Analytics Cloud設定時使用。 如果已安裝`we-gov-forms.ui.analytics-<version>.zip`套件，則此功能為true。 不過，AEM Sites資料需要主動式雲端設定。
...
若要使用Adobe Forms來安裝和設定AEM Forms，「轉換」工具的使用者必須具備下列條件：
...
設定服務以正確與表單轉換工具通訊，使用者必須設定Identity Management系統(IMS)服務以註冊Adobe I/O。
...
>[!NOTE]
>
>本節中建立的憑證將用於在Adobe I/O中建立整合服務。當使用者已建立整合服務時，使用者可以使用Adobe I/O中的該資訊來完成設定。
...
1. 在Experience Cloud區段中，選取Forms轉換工具。
...
1. 系統的服務URL已設定為`https://aemformsconversion.adobe.io/`。
...
1. 請確定範本資料夾已啟用每個人的`jcr:read`許可權。 如果未設定許可權，服務使用者將無法從存放庫讀取範本，並且轉換失敗。
...
此套件中包含的可編輯範本也預先設定了範本/頁面原則，這些原則使用Bootstrap 4 CSS類別進行分頁、樣式設定等。 並非所有類別都已新增至範本原則，但Bootstrap 4支援的任何類別都可以新增至原則。 如需可用類別的清單，請參閱下列內容：
...
另請參閱[範本自訂樣式系統](../../forms/using/forms-install-configure-gov-reference-site.md#customizetemplates)，以取得支援樣式的資訊。
...
您可以根據特定使用案例自訂這些表單。 請勿編輯某些欄位和提交邏輯以確保表單繼續正常運作，例如：

**健康情況權益註冊應用程式：**

* contact_id — 在提交期間用來接收MS® Dynamics聯絡人ID的隱藏欄位
* 提交 — 提交按鈕邏輯需要自訂才能支援回呼。 本檔案說明自訂。 不過，您必須撰寫大型指令碼來提交表單，並透過Forms資料模型對MS® Dynamics執行`POST`和`GET`作業。
...
註冊最適化Forms已提交至OSGI工作流程進行處理。 此工作流程位於&#x200B;*https://&lt;aemserver>：&lt;port>/conf/we-gov/settings/models/we-gov-process.html*。
...
* (AGRS-159)目前的MS® Dynamics FDM需要執行兩項作業，首先，將註冊最適化表單資料發佈至Dynamics，然後擷取使用者記錄以擷取聯絡人ID。 在其目前狀態下，如果Dynamics中出現兩個以上具有相同名稱的使用者，則擷取連絡人ID會失敗，不允許提交註冊最適化表單。
...
安裝後，在Chrome瀏覽器中載入您要測試的頁面。 開啟多個標籤可能會影響您的分數。 因此，最好只開啟一個標籤。 載入頁面後，在頁面上按一下&#x200B;**右鍵**，然後選取&#x200B;**稽核**&#x200B;索引標籤。 開發人員可在此處選取協助工具外掛程式執行的稽核型別。 選取想要的選項後，使用者按一下&#x200B;**產生報表**&#x200B;以建立PDF檔案。 PDF會顯示整體協助工具評等，以及整體提升協助工具評等所用的專案。
...
如果您要匯出此資料，請按一下畫面右側的三個按鈕，然後從可用的選項中選取。
...
`we-gov-forms.pkg.all-<version>.zip`個可安裝的ZIP檔案。 此套件是使用CRX安裝。

封裝管理員使用者可以瀏覽至&#x200B;**Forms** > **主題** > **參考主題** > **可存取的Ultramarine**，存取AEM Forms中的Ultramarine主題。
...
您即將探索We.Gov參考網站。 如需We.Gov參考網站工作流程和步驟的詳細資訊，請參閱[We.Gov參考網站逐步說明](../../forms/using/forms-gov-reference-site-user-demo.md)。
