...
description: 瞭解如何設定EMC Documentum聯結器，以啟用AEM表單與EMC Documentum之間的通訊。
contentOwner: admin
...
>[!NOTE]
>
>在舊版中，資產可以儲存在ECM存放庫中。 在目前版本中，資產儲存在AEM表單原生存放庫中，且存放庫提供者服務已過時。 資產從ECM存放庫移轉至AEM表單存放庫會在您執行升級為AEM表單時完成。 如需詳細資訊，請參閱應用程式伺服器的AEM表單升級指南。
