...
# Adobe Experience Manager6.5 LTS, SP2的本發行說明 {#release-notes}
...
AEM 6.5 LTS SP2現在包含[內容片段和模型管理](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/65lts/)和[啟動](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/launches/)的OpenAPI。 這些API提供了對內容片段的訪問以及用於創作和計畫的啟動。 他們使用與雲服務相同的現代OpenAPI和AEM。
...
* 消除了「站點」編輯器中導致頁面交互過程中行為不一致和中斷標準創作工作流的問題。 該缺陷導致意外的UI響應干擾了元件配置和內容更新。 此更新可穩定受影響的功能，並恢復跨頁面可靠執行編輯動作。 (SITES-35225)
* 消除了「站點」創作介面中的缺陷，該缺陷導致頁面編輯期間行為不一致，並中斷了正常工作流。 作者遇到了意外的UI響應，這些響應干擾了元件交互和內容更新。 此更新可穩定受影響的功能，並在編輯方案中恢復可靠、可預測的行為。 (SITES-35224)
* AEM Sites現在在影像上包括`alt`個文本支援，以滿足ADA和WCAG要求。 頁面輸出不再省略`alt`屬性，因此熒幕助讀程式會收到正確的替代文字。 (SITES-27153)
...
* 更正了側軌資產中的焦點排序。 鍵盤使用者現在透過一致的索引標籤路徑觸及每個資產動作，包括編輯。 (SITES-25360)
* 「搜索資產」模式中320px視區寬度的固定佈局溢出。 模式內容現在會重新流動，並保持可讀性，因此控制項不再重疊或溢出對話框。 (SITES-25330)
* 已更正「編輯」按鈕的NVDA輸出。 NVDA現在將宣佈「編輯」操作，而不是「已按預覽按鈕」。 (SITES-25320)
* 已修復未命名的人口統計工具欄文本輸入，導致無提示或一般螢幕閱讀器輸出。 現在，每個輸入都公開一個基於標籤的可訪問名稱，這改進了鍵盤和輔助技術導航。 (SITES-25316)
...
* 解決註解覆蓋中焦點順序不正確的問題。 鍵盤頁籤現在在覆蓋控制項和注釋項之間循環。 父頁不再從覆蓋後面取焦點。 (SITES-25282)
* 「固定色板」跨距焦點處理。 現在，該對話框將焦點移到清除的標題，並在該入口點開始螢幕閱讀器輸出。 NVDA不再以非順序讀取完整的對話方塊內容。 (SITES-25275)
...
* 在鍵盤導航期間，「設定日期」和「退出時間曲線」連結缺少可見的焦點指示器。 UI現在呈現一種不同的高對比度焦點樣式，以便用戶一眼就能識別活動連結。 (SITES-25232)
* 「預告模式」標頭不再阻止鍵盤用戶移動對話框。 鍵盤控制項現在允許拾取、移動和丟棄操作，這提高了螢幕閱讀器的可用性和總體可操作性。 (SITES-25226)
...
* 清單視圖表頭現在通過ARIA公開排序狀態。 螢幕閱讀器在列排序操作後宣佈升序或降序。 (SITES-24943)
* 現AEM在，在更改文本間距時，在「卡視圖」中保留「更多操作」菜單標籤可見性。 菜單選項保留完整的文本，包括「快速Publish」，並且在任何WCAG文本間距設定期間菜單都保持清晰。 (SITES-24941)
* 「卡操作」菜單欄現在在「卡視圖」中顯示可訪問的名稱。 螢幕閱讀器明確地宣佈菜單欄的用途，語音控制可以按名稱來指定控制項。 (SITES-24938)
...
* 用戶配置滑軌後，左滑軌導航現在遵循預期的鍵盤焦點順序。 Tab焦點會出現在選取的左側邊欄區域，而非Switch Display，這可改善熒幕助讀程式導覽的清晰度。 (SITES-24754)
* 修正「使用者偏好設定」強制回應視窗中色票導覽期間不正確的NVDA意見。 NVDA現在會讀取接收焦點的色票標籤，如此可移除誤導性的色彩輸出。 色板集現在支援一致的鍵盤導航和清晰的選擇感知。 (SITES-24739)
* 已減少`Spin`控制項的詳細NVDA輸出。 已刪除重複輸入標籤的冗餘組標籤，因此NVDA將宣佈控制名稱一次。 鍵盤和螢幕閱讀器導航現在可以發出單一、清晰的通知。 (SITES-24725)
* 現在，「旋轉軸」(Carousel)對話框將焦點放在對話框標題而不是「項目」(Items)頁籤。 取消和Esc將焦點恢復到啟動對話框的控制項，這將減少冗餘NVDA輸出。 (SITES-24716)
* 連結選擇對話框現在將寫程式標籤與上一級樹項的螢幕標籤對齊。 箭頭鍵導覽會針對每個專案觸發可靠的熒幕助讀程式宣告，並移除誤導性的標籤輸出。 (SITES-24710)
...
* 參考邊欄強制回應對話方塊現在會設陷鍵盤焦點。 Tab和Shift+Tab位於對話框控制項內，焦點不再轉義到頁面內容。 螢幕閱讀器僅宣佈對話框內容。 (SITES-24683)
* 「超連結路徑選擇」(Hyperlink Path Selection)模式現在將焦點設定在開啟時的對話框標題上。 取消關閉對話方塊並將焦點恢復到「開啟選取對話方塊」按鈕，可防止焦點遺失及多餘的熒幕閱讀器輸出。 (SITES-24672)
...
* 內容樹現在將鍵盤焦點路由到活動樹項或第一樹項。 樹容器不再充當空的Tab站點，從而防止了Shift+Tab焦點陷阱。 (SITES-1577)
...
* 內容片段編輯器現在將「標籤」標籤本地化。 編輯器還對「集合」標籤進行本地化，因此UI文本與所選區域設定匹配。 (SITES-977)


#### [!DNL Content Fragments] — 片段編輯器{#sites-fragments-editor-65-lts-sp2}

* 當重構後功能切換仍處於禁用狀態時，內容片段變體標籤消失。 即使當切換停止時，該修復程式仍恢復變體標籤支援。 作者可以再次在內容片段編輯器中添加和查看變體標籤。 (SITES-38682)關鍵
...
* 「關聯內容」面板不再呈現硬式編碼的「未定義」預留位置。 內容片段編輯器現在通過本地化資源解析該文本，因此編輯器可以查看已翻譯的UI文本。 (SITES-33675)
...
* 內容片段編輯器現在跨語言環境顯示已翻譯的「常規」頁籤標籤。 編輯器將替換未本地化的頁籤文本，並從頁籤標題中刪除內部ID。 (SITES-30715)
* 內容片段編輯器現在顯示允許的資產類型的已翻譯名稱。 當作者配置內容引用限制時，選取器清單不再混合使用內部字串和僅英語標籤。 (SITES-29699)

#### [!DNL Content Fragments] -GraphQLAPI {#sites-graphql-api-65-lts-sp2}
...
* GraphQL資源管理器在配置瀏覽器名稱包含CJK字元時不再失敗。 終結點建立和保存的查詢訪問正常工作，且「GraphQL查詢編輯器」頁不出錯。 (SITES-31616)
...
功能切換缺陷錯誤報告的頁面管理API狀態。 此更新會新增專用的啟用旗標，並與現有切換一併評估。 頁面管理API現在顯示穩定狀態。 站點管理API仍然是實驗性的。 (SITES-39284)

#### 核心後端{#sites-core-backend-65-lts-sp2}

* 網站製作體驗中的變更，可解決中斷標準頁面編輯工作流程的不一致行為。 作者在元件互動期間遇到非預期的結果，這會干擾內容更新並降低可靠性。 更改可恢復穩定的編輯器行為，並確保在受影響的情形中一致地執行創作操作。 (SITES-35162)嚴重
...
* 即使產品沒有在觸控式UI中公開內容片段啟動，啟動UI仍顯示內容片段啟動控制項。 此變更會從cq-launches-content中移除內容片段啟動程式碼路徑，並調整Launch清單篩選條件。 作者現在可以看到沒有內容片段啟動條目的一致頁面啟動選項。 (SITES-35633)
* 6AEM.5 LTS Quickstart缺少所需的啟動捆綁包和先決條件，這阻止了啟動OpenAPI啟用。 此更新添加了啟動捆綁包和所需的依賴項，如度量支援、 DAM-cfm更新和隊列配置。 啟動API，現在在6.5 LTS Quickstart上運行，並提供所需的運行時元件。 (SITES-35297)
* CF公司推出的打包服務將更新的依賴版本和不必要的GraphQL庫拉回來，AEM使6.5 LTS的整合變得複雜。 此更改將依賴項版本與6.AEM5 LTS基線對齊，並刪除未使用的GraphQL依賴項。 套件組合解析度現在會維持一致，且CF Launches啟動會維持穩定。 (SITES-35295)
* AEM Launches現在會執行6.5 LTS分支的專用Jenkins管道。 管道會在夜間執行，並透過電子郵件傳送失敗警報。 此設定會增加測試涵蓋範圍並及早擷取回歸。 (SITES-35293)
* AEM 6.5 LTS現在隨校準的成品版本推出更新的Launches API套件。 該捆綁包在保持正確的6.5 LTS版本的同時跟蹤主代碼行。 此更新可穩定啟動6.5 LTS堆棧中的API消耗。 (SITES-35292)
* AEM 6.5 LTS現在包含更新的啟動核心套件組合與對齊的相依性版本。 此更新新增片段UUID和參考UUID資料型別的啟動核心處理。 啟動處理現在可在啟動和內容片段工作流中保持一致的行為。 (SITES-35290)
* 優化「站點」編輯器，以解決中斷正常頁面創作工作流的不一致行為。 作者遇到了意外的元件交互，這會干擾內容更新並降低編輯可靠性。 該更改可恢復一致的UI狀態管理，並確保在受影響的情形中可預知地執行創作操作。 (SITES-35138)
* 啟動「編輯」現在顯示本地化錯誤文本，而不是硬編碼的`Provided path is not a launch`字串。 當「編輯」收到無效的啟動路徑時，UI現在將跨語言呈現已翻譯的消息。 (SITES-33360)
* AEM 6.5 LTS現在包含啟動OpenAPI側端連線埠工作。 此更新將Launches API套件組合、內容套件和必要的Quickstart成品帶入同位檢查，並透過穩定的CI驗證啟用內容片段啟動OpenAPI案例。 (SITES-32050)
* 現在啟動UI將本地化「已覆蓋的模板」標籤。 模板覆蓋詳細資訊現在顯示已翻譯文本，而不是只顯示英文字串。 (SITES-29525)
* 已AEM解決&#x200B;**站點** > **啟動** > **編輯**&#x200B;中缺少的本地化鍵。 用戶現在看到已轉換的錯誤消息，而不是原始的「無法更新啟動源清單」字串。 (SITES-21499)
...
* OSGi預設配置設定為`preview=true`，並強制通用編輯器以預覽模式啟動。 此更新會更正預設值並恢復標準的生產專案行為。 現在，除非管理員顯式啟用預覽模式，否則通用編輯器將在生產模式下開啟。 (SITES-37193)
...
Assets Relate現在適用於包含空格的檔案名稱。 更新的「關聯」客戶端邏輯現在可以正確處理包含空間的路徑，並避免在關係選擇期間出現`undefined`個源錯誤。 「關係」對話方塊現在會開啟，並儲存關係，而不使用UI掛起或旋轉器。 DAM使用者無需重新命名檔案，即可建立、衍生及解除資產的相關性。 (Assets-56418)
...
* 現AEM在避免生成的`contributor`中`WEB-INF/resources/provisioning/model.txt`組的重複ACL項。 WAR輸出現在包含一個一致的ACL塊，可防止在審閱期間出現混淆的權限差異。 (GRANITE-63269)
* 在包AEM刷新操作期間不再清除反序列化防火牆阻止清單和允許清單設定。 更新的篩選器註冊邏輯使活動防火牆實例與已保存的配置保持一致，因此在不重新啟動的情況下保持啟用保護。 (GRANITE-61382)
* 在`NullPointerException`訪問期間，Felix Web Console不再引發間歇性的`/system/console`錯誤。 更新的ServiceTracker處理會阻止空跟蹤器狀態。 在重複請求和自動驗證期間，主控台登入和導覽會保持穩定。 (GRANITE-61042)
...
* Sling更新管理登錄允許清單以使用包含的術語和新配置PID。 此更改與Sling JCR Base 3.2.0一致。 (GRANITE-63756)
...
由AEM於XLIFF格式問題，Java 17或Java 21上的翻譯不再失敗。 匯出管道現在會產生符合翻譯提供者所接受的標準XLIFF。 此更改消除了翻譯作業中斷並恢復了翻譯服務和翻譯服務之AEM間的可預測切換。 支援的Java執行階段中，翻譯工作流程現在保持穩定。 (CQ-4360217)
...
適用於AEM 6.5 LTS SP2的UberJar使用AEM 6.5 LTS UberJar 6.6.0版。您可以從Maven中央存放庫擷取對應的UberJar成品。 與AEM 6.5不同，AEM 6.5 LTS將公用API和已過時的API分隔成兩個不同的成品。
...
另請參閱[更新AEM Uber Jar版本](/help/sites-deploying/upgrading-code-and-customizations.md#update-the-aem-uber-jar-version)。

### 升級 {#upgrade}

* 如需升級程序的詳細資訊，請參閱[升級文件](/help/sites-deploying/upgrade.md)。
* 有關詳細升級說明，請參閱JEE[上的](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-65-lts/content/forms/upgrade-aem-forms/upgrade)AEM Forms6.5 LTS SP1升級指南
...
**環境**
適用於：AEM 6.5 LTS（本地）客戶安裝Service Pack 2(SP2)。 SP2作為快速啟動JAR提供。

**為什麼這很重要**
6.AEM5 LTS的SP2作為快速啟動JAR而不是通過包管理器安裝的ZIP。 內部部署客戶的升級方式是取代 Quickstart JAR，將檔案解壓縮然後重新啟動。此方法與 Adobe 的就地升級程序一致。
...
>[!NOTE]
>
>上面顯示的檔案名（例如， `cq-quickstart-6.6.x.jar`）反映了在此LTS發行版中觀察到的Quickstart項目命名；始終使用從軟體分發下載的準確檔案名。
...
| 區域 | 功能 | 替代方案 | 版本 (SP) |
| --- | --- | --- | --- |
| 快速入門 | 蒙戈APIs | Mongo API現在已棄用，計畫在將來的版本中刪除。 | 6.5 TS SP2 |
...
應用實例時計畫實例的停機時間。 對於離線壓縮，請使用相應的[oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)（也可在軟體分發上使用）。

>[!NOTE]
>
> * 對於任何橡木運行的操作，請使用[橡木運行的1.88.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)。
>
> * 開AEM始設定系統屬性`oak.compaction.legacy=true`。

### 為站點無頭API安裝所需的Oak索引{#site-headless-api}
