...
* 已新增對WebSphere® Liberty Profile (WLP)的支援。 只有Oracle資料庫和IBM® Sumeru JDK 21支援WLP。

* 增強視覺規則編輯器中的使用者體驗。 本次更新內容包括：

  * 若要顯示更新的規則狀態，摘要檢視會在儲存後自動重新載入
...
* 新增下載記錄檔案(DoR) /表單提交為規則編輯器中的標準按鈕。 此增強功能讓客戶不需撰寫自訂程式碼即可使用 downloadDoR 函數，進而提高使用性和效率。 (FORMS-21263)
...
* 修正 Sites 編輯器中導致頁面製作期間中斷預期行為，並造成元件互動不一致的問題。 作者經歷不可靠的使用者介面回應，干擾標準編輯任務並降低工作流程效率。 此更新會改善編輯器邏輯，並還原受影響元件間穩定、可預測的互動。 (SITES-35227)
...
* 鍵盤導覽時，「設定日期」和「退出時間扭曲」連結缺少可見的焦點指示器。 UI現在會呈現不同的高對比焦點樣式，讓使用者可以輕鬆識別作用中的連結。 (SITES-25232)
* Teaser 強制視窗標題不再阻擋鍵盤使用者移動對話框。 鍵盤控制現在允許選擇、移動和放置動作，這可以改善熒幕助讀程式的易用性和整體操作性。 (SITES-25226)
...
* 調整 Sites 製作介面中的元件處理，停止頁面更新期間的不規則行為。 此缺陷導致無法預測編輯器回應，干擾例行內容修改並降低工作流程效率。 此更新會調整編輯器邏輯與預期的互動模式，並在編寫活動期間提供可靠的效能。 (SITES-35078) 關鍵
...
* 當作者使用「促銷活動目前頁面及子頁面」功能推廣子區段時，啟動推廣範圍會發生偏移。 AEM 也會推廣不相關的頁面，並造成非預期的即時網站修改。 此修正會修正Launch範圍計算，因此只會提升所選的子樹狀結構。 (SITES-38315)
...
* 內容片段Launches封裝提取了較新的相依性版本和不必要的GraphQL程式庫，這會使AEM 6.5 LTS整合複雜化。 此變更會將相依性版本與 AEM 6.5 LTS 基準保持一致，並移除未使用的 GraphQL 相依性。 套件解析度現在會保持一致，而內容片段啟動啟動會保持穩定。 (SITES-35295)
* AEM 啟動現在會執行 6.5 LTS 分支的專用 Jenkins 管道。 管道會在夜間執行，並透過電子郵件傳送失敗警報。 此設定可增加測試涵蓋範圍並及早識別回歸。 (SITES-35293)
...
* 若要解決會中斷一般頁面製作工作流程的不一致行為，已最佳化網站編輯器。 作者遇到意外的元件互動，干擾內容更新並降低編輯可靠性。 此變更恢復了一致使用者介面狀態管理，並確保在受影響的情境中，編輯動作能可預測地執行。 (SITES-35138)
...
Assets 「相關」現在適用於包含空格的檔案名稱。 更新「相關」用戶端邏輯現在可以正確處理包含空格的路徑，並避免在相關性選取期間發生 `undefined` 來源錯誤。 「相關」對話框現在會開啟，並儲存關係，而不使用使用者介面下拉式選單。 DAM使用者無需重新命名檔案，即可建立、衍生及移除資產關係。 (Assets-56418)
...
* 使用者在自適應表單儀表板中遇到安全性弱點。 具體來說，在startpointcontrol.js檔案中識別出跨網站指令碼(XSS)問題，此問題會允許執行惡意指令碼。 (FORMS-20679)
...
* 在 Turnkey 模式中，現在會於全新安裝和升級期間正確套用資料庫連接埠更新。 在全新安裝模式中，使用者會從所有可用的連線埠中進行選取，而在升級模式中，在升級期間會正確參考lc_turnkey.xml中更新的資料庫連線埠。 (FORMS-24689)
* 在Linux®上設定JBoss® EAP 8.0時，在Windows上修改的Shell指令碼不再導致/bin/sh^M：錯誤的解譯器或$&#39;\r&#39;：命令找不到，因為CRLF行結尾而發生錯誤。 (FORMS-24688)
* 在 JBoss® EAP 8 上執行的 Forms JEE LTS 部署中，Reader 擴充功能使用者介面可能會因內部伺服器錯誤而失敗。 (FORMS-24894)
* 在Linux®上，當Forms JEE LTS Configuration Manager以未設定或不正確的`OSFileSetIntendedFor`執行時，使用者遇到執行階段或部署問題。 在`configurationManager/config/solcomp/LFS_Foundation.properties`中設定值，這會使設定無法針對Linux®正確自訂。 在完成安裝之後，且執行 Configuration Manager 之前，請於該檔案中設定 `OSFileSetIntendedFor=Linux`。 (FORMS-24741)
...
* JMX主控台和Web主控台現在會傳送主控台CSS資源的Content-Type： text/css標頭。 嚴格的 MIME 檢查不再封鎖樣式工作表載入，因此 `/system/console/jmx` 使用者介面會以一般樣式呈現。 (GRANITE-63677)
...
升級 Service Pack 後開啟 JSP 檔案時，CRXDE Lite 不再顯示空白索引標籤。 AEM現在包含相符的CodeMirror核心和附加程式碼，可避免嚴重的瀏覽器錯誤，並確保編輯器仍能正常運作。 (GRANITE-64333)
...
* Sling 會更新管理登入允許清單，使用內含術語和新設定 PID。 此變更與 Sling JCR Base 3.2.0 一致。 (GRANITE-63756)
  ...
  * 請勿同時執行已棄用的和新的允許清單設定。 混合設定可能會產生模稜兩可的情況並產生非預期的行為。 當您移轉至AEM 6.5 LTS SP2時，請移除已棄用的PID。
