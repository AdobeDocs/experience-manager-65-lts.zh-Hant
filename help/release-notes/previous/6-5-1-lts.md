...
source-git-commit: d3dabe8378457920d3689db92f78a414b8af771d
workflow-type: tm+mt
source-wordcount: '8036'
ht-degree: 89%
...
* **FORMS-24688：**&#x200B;在Linux上設定JBoss EAP 8.0 (AEM Forms 6.5.1 LTS)時，在Windows上修改的指令碼可能包含CRLF行結尾，造成/bin/sh^M：錯誤解譯器或$&#39;\r&#39;：命令找不到錯誤。 使用dos2unix將受影響的.sh檔案轉換為Unix (LF)格式(例如，尋找。 -type f -name &quot;*.sh&quot; -exec dos2unix {} +)以解決問題。

* **FORMS-24689：**&#x200B;在Turnkey升級模式中，不會從UI擷取資料庫連線埠的更新，且必須在lc_turnkey.xml組態檔中手動修改。 如果未正確更新，系統可能會繼續使用舊的連線埠值。

* **FORMS-24687：**&#x200B;在JBoss EAP 8上的AEM Forms 6.5.1 LTS叢集部署中，`domain/configuration/domain_oracle.xml`可能包含重複的`<security>`標籤，導致無效的XML無法啟動網域控制站。
