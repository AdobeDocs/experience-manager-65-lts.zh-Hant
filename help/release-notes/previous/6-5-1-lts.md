...
exl-id: 58aca04b-a3c8-4ef6-bfc0-78e7e7ed5131
source-git-commit: 5bc9407dbe217349378d1d33dada016bbcc4e2c4
workflow-type: tm+mt
source-wordcount: '8476'
ht-degree: 98%
...
>[!IMPORTANT]
>
> **強制 Hotfix** – 若要避免離線壓縮在安裝 Service Pack 1 時發生 SNFE (SegmentNotFoundException) 問題，請安裝[已知問題 – 線上壓縮期間存放庫損毀](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146)中說明的 Hotfix。
...
JEE 上的 AEM 6.5 Forms LTS 現已推出。 如需支援環境的詳細資訊，請參閱[支援的平台](/help/forms/using/aem-forms-jee-supported-platforms.md)組合文件。 安裝程式連結可在 [AEM Forms 發行版本](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases)頁面上取得。

#### AEM Forms 6.5 LTS SP1 包含哪些內容

**Java 支援更新**

已引進對較新 Java 版本的支援：

* Oracle Java™ 17/21 (64 位元) (請參閱 [Java 支援](#java-support)區段的詳細資訊。

**應用程式伺服器支援更新**

* 新增對 JBoss® EAP 8 的支援。
* 舊版 PicketBox 安全性架構已移除。
* Elytron 型的認證存放區現在支援安全的認證管理。

**設定：認證存放區 (Elytron 型)**

JBoss®EAP 8 上的 AEM Forms 使用 Elytron 管理安全認證。 客戶必須設定以 Elytron 為基礎的認證存放區，方能確保伺服器成功啟動及安全資料庫驗證。
...
* 支援 Servlet 規格 5+
* 根據 Jakarta EE 9 法規遵循

**命名空間移轉需求**

* Jakarta EE 9 引入命名空間，從 `javax.*` 變更為 `jakarta.*`
* 所有 **自訂 DSC** 都必須移轉至 `jakarta.*` 命名空間
* AEM Forms 6.5 LTS SP1 僅支援 **Jakarta EE 9+ 應用程式伺服器**

如需詳細資訊，請參閱&#x200B;**從 Javax 移轉至 Jakarta 命名空間**。

#### 從 `javax` 移轉至 `jakarta` 命名空間

從 **AEM Forms 6.5 LTS SP1** 開始，僅支援實施 **Jakarta Servlet API 5/6** 的應用程式伺服器。 透過 **Jakarta EE 9 和更新版本**，所有 API 都會從 `javax.{}` 命名空間轉換為 `jakarta.`。

因此，**所有自訂 DSC 都必須使用 `jakarta` 命名空間**。 使用 `javax.{}` API 建置的自訂元件與支援的應用程式伺服器&#x200B;**不相容**。

**自訂 DSC 的移轉選項**

您可以使用下列其中一種方法移轉現有的自訂 DSC：
...
* 確保與 Jakarta EE 9+ 長期相容
* 最適合主動維護的專案

**選項 2：使用 Eclipse 轉換程式的二進位移轉**

* 使用 **Eclipse 轉換器**&#x200B;工具將編譯的二進位檔 (`.jar`，`.war`) 從 `javax` 轉換為 `jakarta`
* 不需要變更原始程式碼或重新編譯
...
以下是移轉期間所需之命名空間變更的常見範例：

* 移轉前 (Javax)    移轉後 (Jakarta)
* javax.servlet. **jakarta.servlet**
* javax.servlet.http. **jakarta.servlet.http.**

**範例匯入對應**

下方表格顯示從 `javax` 移轉至 `jakarta` 時所需的一般命名空間變更：

| 轉移前 (`javax`) | 轉移後 (`jakarta`) |
| ---------------------- | ------------------------ |
| `javax.servlet.*` | `jakarta.servlet.*` |
| `javax.servlet.http.*` | `jakarta.servlet.http.*` |

在更新自訂 DSC 原始程式碼或驗證轉換後的二進位檔時，使用這些對應做為參考。
...
* 定義出 AEM 6.5 的維護和保障策略。 此修正包含下列內容：
   * Service Pack 發布週期。
   * Hotfix 發布週期。
   * 與 AEM 6.6 並行支援。
   * 已更新支援矩陣。
...
>[!NOTE]
>
> (僅限 Forms) 一次性安裝需要 Oracle Java™ 8/11 (64 位元)。
...
**為什麼這很重要**
EM 6.5 LTS 適用的 SP1 會以 Quickstart JAR 形式提供，而非透過「封裝管理員」進行安裝的 ZIP 檔。 內部部署客戶的升級方式是取代 Quickstart JAR，將檔案解壓縮然後重新啟動。 此方法與 Adobe 的就地升級程序一致。
...
* **FORMS-24688：**&#x200B;在 Linux 上設定 JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) 時，於 Windows 中建立或編輯的指令碼可能包含 CRLF 行結尾，導致「/bin/sh^M: bad interpreter」或「$&#39;\r&#39;: command not found」等錯誤。 確定受影響的檔案使用 LF 行尾。 這可以使用 dos2unix 之類的工具或任何等效的方法來完成。

* **FORMS-24689：**&#x200B;在 Turnkey 模式中，於重新安裝或升級期間未正確套用資料庫連接埠更新。 在全新安裝模式中，使用者只能從螢幕上顯示的連接埠進行選取。 在升級模式中，必須先於 lc_turnkey.xml 中手動更新新的資料庫連接埠，才能繼續升級。

* **FORMS-24687：**&#x200B;在 JBoss EAP 8 上的 AEM Forms 6.5 LTS 叢集部署中，domain/configuration/domain_oracle.xml、domain_mysql.xml 和 domain_mssql.xml 檔案可能包含重複的 `<security>` 標記，導致無效的 XML 無法啟動網域控制站。

### Granite

### 離線壓縮後線上壓縮期間存放庫損毀 (GRANITE-65146) {#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146}

若先前曾在 JCR 存放庫上執行過離線壓縮，使用者在執行線上壓縮時可能會遇到存放庫損毀的情況。 此情境中可能會發生 `SegmentNotFoundException` (SNFE)。 這可能導致存放庫損毀。

若要解決此問題，請透過封裝管理員安裝 [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip) 的 Hotfix。 由於 Hotfix 包含低階 `oak-segment-tar` 組合包，所以執行個體會在安裝後重新啟動。

計劃套用執行個體時的停機時間。 若要離線壓縮，請使用對應的 [Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (也可在 Software Distribution 中使用)。

>[!NOTE]
>
> 對於任何 Oak 執行的作業，請使用 [Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (連結在上方)。
...
### AEM 6.5 LTS SP1中缺少`com.adobe.granite.apicontroller`套件(GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

AEM 6.5 LTS SP1中缺少`com.adobe.granite.apicontroller`套件。 此套件組合會控制OSGi套件組合解析的方式，並可防止套件組合解析為其他套件，這對於限制公開的API很有用。

從[軟體發佈](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip)安裝Hotfix以使用此功能。
...
* [ Experience Manager 6.5 LTS Service Pack 1 包含的 OSGi 套件清單](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5 LTS Service Pack 1 中包含的內容套件清單](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
