...
>[!IMPORTANT]
>
> **強制Hotfix** — 若要在安裝Service Pack 1時避免離線壓縮發生SNFE (SegmentNotFoundException)問題，請安裝[已知問題 — 線上壓縮期間存放庫損毀](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146)中說明的Hotfix。
...
* Oracle Java™ 17/21 （64位元） (請參閱「[Java支援](#java-support)」一節的詳細資訊。
...
>[!NOTE]
>
> （僅限Forms）一次性安裝需要Oracle Java™ 8/11 （64位元）。
...
**環境**
適用於：安裝Service Pack 1 (SP1)的AEM 6.5 LTS （內部部署）客戶。 SP1 會以 Quickstart JAR 形式提供。

**這很重要的原因**
AEM 6.5 LTS的SP1會以Quickstart JAR形式出貨，而非透過「封裝管理員」安裝的ZIP。 內部部署客戶的升級方式是取代 Quickstart JAR，將檔案解壓縮然後重新啟動。 此方法與 Adobe 的就地升級程序一致。
...
* **FORMS-24688：**&#x200B;在Linux上設定JBoss EAP 8.0 (AEM Forms 6.5.1 LTS)時，在Windows中建立或編輯的指令碼可能包含CRLF行結尾，導致/bin/sh^M：解譯錯誤或$&#39;\r&#39;：命令找不到錯誤。 確定受影響的檔案使用LF行尾。 這可以使用dos2unix之類的工具或任何等效的方法來完成。

* **FORMS-24689：**&#x200B;在Turnkey模式中，在重新安裝或升級期間未正確套用資料庫連線埠更新。 在全新安裝模式中，使用者只能從熒幕上顯示的連線埠進行選取。 在升級模式中，必須先在lc_turnkey.xml中手動更新新的資料庫連線埠，才能繼續升級。

* **FORMS-24687：**&#x200B;在JBoss EAP 8上的AEM Forms 6.5 LTS叢集部署中，domain/configuration/domain_oracle.xml、domain_mysql.xml和domain_mssql.xml檔案可能包含重複的`<security>`標籤，導致無效的XML無法啟動網域控制站。

### Granite

### 離線壓縮後線上壓縮期間存放庫損毀(GRANITE-65146) {#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146}

如果先前在JCR存放庫上執行離線壓縮，使用者會線上上壓縮期間遇到存放庫損毀的問題。 此情境中可能會發生`SegmentNotFoundException` (SNFE)。 這可能導致存放庫損毀。

若要解決此問題，請透過[Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip)的封裝管理員安裝Hotfix。 由於Hotfix包含低階`oak-segment-tar`套件組合，所以執行個體會在安裝後重新啟動。

套用執行個體時，針對執行個體的停機時間進行規劃。 若要離線壓縮，請使用對應的[Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) （也可在Software Distribution上取得）。

>[!NOTE]
>
> 對於任何Oak執行的作業，請使用[Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) （連結在上方）。
