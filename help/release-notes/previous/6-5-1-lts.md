...
* **FORMS-24688：**&#x200B;在Linux上設定JBoss EAP 8.0時，在Windows上修改的Shell指令碼可能包含CRLF行結尾，造成`/bin/sh^M: bad interpreter`或$`\r`：命令找不到錯誤。 使用dos2unix將受影響的檔案轉換為LF行尾以解決此問題。

* **FORMS-24689：**&#x200B;在Turnkey升級模式中，在升級期間可能無法正確套用`lc_turnkey.xml`中的資料庫連線埠，導致處理序參考舊的連線埠值。
