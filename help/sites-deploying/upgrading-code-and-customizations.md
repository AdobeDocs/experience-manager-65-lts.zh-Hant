...
如上所述及下圖所示，在第一個步驟中執行[模式偵測器](/help/sites-deploying/pattern-detector.md)可協助您評估升級的整體複雜性。 它也可以協助您決定您是要以相容性模式執行，或更新自訂以使用所有新的AEM 6.5功能。 如需詳細資訊，請參閱AEM 6.5](/help/sites-deploying/backward-compatibility.md)中的[回溯相容性頁面。
...
傳統UI編寫仍可在AEM 6.5中使用，但已過時。 如果您的應用程式在傳統UI編寫環境中執行，建議升級至AEM 6.5並繼續使用傳統UI。 移轉至Touch UI可計畫為單獨的專案，以便透過數個開發週期完成。 若要在AEM 6.5中使用傳統UI，必須將數個OSGi設定認可至程式碼基底。 在[啟用傳統UI的存取](/help/sites-administering/enable-classic-ui.md)下可找到如何進行設定的詳細資訊。
