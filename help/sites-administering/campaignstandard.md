...
不過，必須先設定密碼，AEM才能使用`aemserver`使用者。 這不能透過Adobe Campaign完成。 必須由Adobe支援工程師執行。 [向Adobe客戶服務提交票證](https://experienceleague.adobe.com/?support-tab=home#support)以要求重設`aemserver`密碼。 取得Adobe客戶服務的密碼後，請妥善儲存在安全位置。
